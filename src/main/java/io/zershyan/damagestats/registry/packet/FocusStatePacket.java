...
public record FocusStatePacket(
    ...
    public static final StreamCodec<FriendlyByteBuf, FocusStatePacket> STREAM_CODEC =
    ...
    private static void encode(FriendlyByteBuf buf, FocusStatePacket packet) {
        buf.writeEnum(packet.result);
        FocusSummary.STREAM_CODEC.encode(buf, packet.summary);
        buf.writeVarInt(packet.requestId);
    }

    private static FocusStatePacket decode(FriendlyByteBuf buf) {
